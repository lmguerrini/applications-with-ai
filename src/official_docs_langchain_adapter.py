...
def run_langchain_official_docs_lookup(
    *,
    request: OfficialDocsLookupRequest,
    settings: Settings,
    mcp_call_fn: MCPRequestFn,
) -> OfficialDocsLookupResult:
    tools_result = mcp_call_fn(
        server_url=settings.official_langchain_docs_mcp_url,
        method="tools/list",
        params={},
        timeout_seconds=settings.official_docs_timeout_seconds,
    )
...
def _build_langchain_documents_from_entries(
    ...
    for entry in raw_entries:
        ...
        title = _first_langchain_text(entry, ("title", "name"))
        url = _first_langchain_text(entry, ("url", "link"))
...
def _extract_langchain_snippet_texts(entry: Mapping[str, object]) -> list[str]:
    snippet_list = _extract_langchain_snippet_list(entry.get("snippets"))
    if snippet_list:
        return snippet_list

    direct_content = _first_langchain_text(
        entry,
...
def _extract_langchain_flattened_field_value(text: str, field_name: str) -> str | None:
    ...
    for raw_line in text.splitlines():
        ...
        if not stripped_line.startswith(prefix):
            continue
        cleaned_value = _clean_langchain_text(stripped_line.removeprefix(prefix))
        if cleaned_value is not None:
            return cleaned_value
    return None


def _extract_langchain_snippet_list(value: object) -> list[str]:
    ...
    for item in value:
        if isinstance(item, str):
            cleaned = _clean_langchain_text(item)
            if cleaned is not None:
                snippets.append(cleaned)
        elif isinstance(item, Mapping):
            cleaned = _clean_langchain_text(item.get("text"))
            if cleaned is not None:
                snippets.append(cleaned)
    return snippets


def _first_langchain_text(entry: Mapping[str, object], keys: tuple[str, ...]) -> str | None:
    for key in keys:
        cleaned = _clean_langchain_text(entry.get(key))
        if cleaned is not None:
            return cleaned
    return None


def _clean_langchain_text(value: object) -> str | None:
